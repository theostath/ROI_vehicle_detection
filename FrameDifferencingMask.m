function [ROI_mask,frames_roi] = FrameDifferencingMask(frames,Threshold)
...
for i = 1:300
    if i == 300 % eidikh periptwsh gia to teleutaio frame pou den exei epomeno
        ROI_mask(:,:,i) = ROI_Mask(gray_frames(:,:,i-1),gray_frames(:,:,i),Threshold);
    else
        ROI_mask(:,:,i) = ROI_Mask(gray_frames(:,:,i),gray_frames(:,:,i+1),Threshold);
    end
end
